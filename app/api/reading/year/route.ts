/**
 * POST /api/reading/year — leitura de ano offline (mapa + narrativa + action).
 */

import { NextResponse } from "next/server";
import { getYearReading } from "@/lib/readings/yearReading";
...
export async function POST(req: Request) {
  ...
  const { map, reading, action } = await getYearReading(profile);
